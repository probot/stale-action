...
const createErrorAnnotation = require('./create-error-annotation')
...
module.exports = class Stale {
  constructor (tools, config) {
    ...
    if (error) {
      createErrorAnnotation(tools, error).then(() => {
        tools.log.fatal('Sent annotation for invalid config!')
  ...
  async markAndSweep (type) {
    ...
    if (daysUntilClose) {
      ...
      return this.log.info('Configured to leave stale issues open')
    }
  }
  ...
  async unmark (type, issue) {
    ...
    if (perform) {
      this.log.info({
        prefix: '[Stale]',
        message: `${owner}/${repo}#${number} is being unmarked`,
        suffix: '(unmark)'
      })
      if (unmarkComment) {
        await this.github.issues.createComment({
      ...
      return this.github.issues
        ...
        .then(() => {
          this.log.complete({
            prefix: '[Stale]',
            message: `${owner}/${repo}#${number} was unmarked.`
          })
        })
        .catch(err => {
      ...
      return this.log.complete({
        prefix: '[Stale]',
        message: `${owner}/${repo}#${number} would have been unmarked.`,
        suffix: '(dry-run)'
      })
    }
  }
  ...
  async ensureStaleLabelExists (type) {
    ...
    return this.tools.github.issues
      .getLabel({ owner, repo, name: staleLabel })
      .catch(() => {
        return this.tools.github.issues.createLabel({
          owner,
          repo,
          name: staleLabel,
          color: 'ffffff'
